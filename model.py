...
class _bn_relu_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        print("****", np.max(x.cpu().numpy()), np.min(x.cpu().numpy()), np.mean(x.cpu().numpy()),
              np.std(x.cpu().numpy()), x.shape)
        for i, layer in enumerate(self.model):
            ...
            print("____", np.max(x.cpu().numpy()), np.min(x.cpu().numpy()), np.mean(x.cpu().numpy()),
                  np.std(x.cpu().numpy()), x.shape)
            print(x[0])
        return x
...
class res_skip(nn.Module):
    ...
    def __init__(self):
        ...
        self.block0 = _residual_block(in_filters=3, nb_filters=24, repetitions=2, is_first_layer=True)  # (input)
